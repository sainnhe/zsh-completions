...
'--no-check=[Skip type-checking. If the value of "remote" is supplied, diagnostic errors from remote modules will be ignored]::NO_CHECK_TYPE:_default' \
...
'--node-modules-dir=[Sets the node modules management mode for npm packages]::MODE:_default' \
'--vendor=[Toggles local vendor folder usage for remote modules and a node_modules folder for npm packages]:: :(true false)' \
...
'*-r+[Reload source code cache (recompile TypeScript)   no value                                                 Reload everything   jsr\:@std/http/file-server,jsr\:@std/assert/assert-equals  Reloads specific modules   npm\:                                                     Reload all npm modules   npm\:chalk                                                Reload specific npm module]::CACHE_BLOCKLIST:_files' \
'*--reload=[Reload source code cache (recompile TypeScript)   no value                                                 Reload everything   jsr\:@std/http/file-server,jsr\:@std/assert/assert-equals  Reloads specific modules   npm\:                                                     Reload all npm modules   npm\:chalk                                                Reload specific npm module]::CACHE_BLOCKLIST:_files' \
'--lock=[Check the specified lock file. (If value is not provided, defaults to "./deno.lock")]::FILE:_files' \
'--frozen=[Error out if lockfile is out of date]::BOOLEAN:(true false)' \
'--cert=[Load certificate authority from PEM encoded file]:FILE:_files' \
'--unsafely-ignore-certificate-errors=[DANGER\: Disables verification of TLS certificates]::HOSTNAMES:_default' \
...
'-P+[]:: :_default' \
'--permission-set=[]:: :_default' \
'*-R+[]::PATH:_files' \
'*--allow-read=[]::PATH:_files' \
'*--deny-read=[]::PATH:_files' \
'*--ignore-read=[]::PATH:_files' \
'*-W+[]::PATH:_files' \
'*--allow-write=[]::PATH:_files' \
'*--deny-write=[]::PATH:_files' \
'-N+[]::IP_OR_HOSTNAME:_default' \
'--allow-net=[]::IP_OR_HOSTNAME:_default' \
'--deny-net=[]::IP_OR_HOSTNAME:_default' \
'-E+[]::VARIABLE_NAME:_default' \
'--allow-env=[]::VARIABLE_NAME:_default' \
'--deny-env=[]::VARIABLE_NAME:_default' \
'--ignore-env=[]::VARIABLE_NAME:_default' \
'-S+[]::API_NAME:_default' \
'--allow-sys=[]::API_NAME:_default' \
'--deny-sys=[]::API_NAME:_default' \
'--allow-run=[]::PROGRAM_NAME:_default' \
'--deny-run=[]::PROGRAM_NAME:_default' \
'*--allow-ffi=[]::PATH:_files' \
'*--deny-ffi=[]::PATH:_files' \
'-I+[Allow importing from remote hosts. Optionally specify allowed IP addresses and host names, with ports as necessary. Default value\: deno.land\:443,jsr.io\:443,esm.sh\:443,raw.esm.sh\:443,cdn.jsdelivr.net\:443,raw.githubusercontent.com\:443,gist.githubusercontent.com\:443]::IP_OR_HOSTNAME:_default' \
'--allow-import=[Allow importing from remote hosts. Optionally specify allowed IP addresses and host names, with ports as necessary. Default value\: deno.land\:443,jsr.io\:443,esm.sh\:443,raw.esm.sh\:443,cdn.jsdelivr.net\:443,raw.githubusercontent.com\:443,gist.githubusercontent.com\:443]::IP_OR_HOSTNAME:_default' \
'--deny-import=[Deny importing from remote hosts. Optionally specify denied IP addresses and host names, with ports as necessary.]::IP_OR_HOSTNAME:_default' \
'--inspect=[Activate inspector on host\:port \[default\: 127.0.0.1\:9229\]. Host and port are optional. Using port 0 will assign a random free port.]::HOST_PORT:_default' \
'--inspect-brk=[Activate inspector on host\:port, wait for debugger to connect and break at the start of user script]::HOST_PORT:_default' \
'--inspect-wait=[Activate inspector on host\:port and wait for debugger to connect before running user code]::HOST_PORT:_default' \
'--inspect-publish-uid=[]:VALUE:_default' \
'*--allow-scripts=[Allow running npm lifecycle scripts for the given packages   Note\: Scripts will only be executed when using a node_modules directory (\`--node-modules-dir\`)]::PACKAGE:_default' \
'--location=[Value of globalThis.location used by some web APIs]:HREF:_urls' \
'--v8-flags=[To see a list of all available flags use --v8-flags=--help   Flags can also be set via the DENO_V8_FLAGS environment variable.   Any flags set with this flag are appended after the DENO_V8_FLAGS environment variable]::V8_FLAGS:_default' \
'--seed=[Set the random number generator seed]:NUMBER:_default' \
'--trace-ops=[Trace low-level op calls]::OPS:_default' \
...
'(--no-check)--check=[Enable type-checking. This subcommand does not type-check by default   If the value of "all" is supplied, remote modules will be included.   Alternatively, the '\''deno check'\'' subcommand can be used]::CHECK_TYPE:_default' \
'*--watch=[Watch for file changes and restart process automatically.   Local files from entry point module graph are watched by default.   Additional paths might be watched by passing them as arguments to this flag.]::FILES:_files' \
'(--watch)*--watch-hmr=[Watch for file changes and restart process automatically.   Local files from entry point module graph are watched by default.   Additional paths might be watched by passing them as arguments to this flag.]::FILES:_files' \
'*--watch-exclude=[Exclude provided files/patterns from watch mode]::FILES:_files' \
'--ext=[Set content type of the supplied file]: :(ts tsx js jsx mts mjs cts cjs)' \
'*--env-file=[Load environment variables from local file   Only the first environment variable with a given key is used.   Existing process environment variables are not overwritten, so if variables with the same names already exist in the environment, their values will be preserved.   Where multiple declarations for the same environment variable exist in your .env file, the first one encountered is applied. This is determined by the order of the files you pass as arguments.]::FILE:_files' \
'(--inspect --inspect-wait --inspect-brk)--coverage=[Collect coverage profile data into DIR. If DIR is not specified, it uses '\''coverage/'\''.   This option can also be set via the DENO_COVERAGE_DIR environment variable.]::DIR:_files' \
'--cpu-prof-dir=[Directory where the V8 CPU profiles will be written. Implicitly enables --cpu-prof]:DIR:_files -/' \
'--cpu-prof-name=[Filename for the CPU profile (defaults to CPU.<timestamp>.<pid>.cpuprofile)]:NAME:_default' \
'--cpu-prof-interval=[Sampling interval in microseconds for CPU profiling (default\: 1000)]:MICROSECONDS:_default' \
'-t+[Execute tasks with a tunnel to Deno Deploy.      Create a secure connection between your local machine and Deno Deploy,     providing access to centralised environment variables, logging,     and serving from your local environment to the public internet]:: :(true false)' \
'--tunnel=[Execute tasks with a tunnel to Deno Deploy.      Create a secure connection between your local machine and Deno Deploy,     providing access to centralised environment variables, logging,     and serving from your local environment to the public internet]:: :(true false)' \
'*-h+[]::CONTEXT:(unstable full)' \
'*--help=[]::CONTEXT:(unstable full)' \
...
'--cpu-prof[Start the V8 CPU profiler on startup and write the profile to disk on exit. Profiles are written to the current directory by default]' \
'--cpu-prof-md[Generate a human-readable markdown report alongside the CPU profile]' \
'--cpu-prof-flamegraph[Generate an SVG flamegraph alongside the CPU profile]' \
...
'*--ignore=[Ignore advisories matching the given CVE IDs]:CVE:_default' \
'--unstable[The \`--unstable\` flag has been deprecated. Use granular \`--unstable-*\` flags instead   To view the list of individual unstable feature flags, run this command again with --help=unstable]' \
'--unstable-bare-node-builtins[Enable unstable bare node builtins feature]' \
'--unstable-broadcast-channel[Enable unstable \`BroadcastChannel\` API]' \
'--unstable-bundle[Enable unstable bundle runtime API]' \
...
'*--preload=[A list of files that will be executed before the main module]:FILE:_files' \
'*--require=[A list of CommonJS modules that will be executed before the main module]:FILE:_files' \
'(--no-check)--check=[Set type-checking behavior. This subcommand type-checks local modules by default, so adding --check is redundant   If the value of "all" is supplied, remote modules will be included.   Alternatively, the '\''deno check'\'' subcommand can be used]::CHECK_TYPE:_default' \
...
'--inline-imports=[Whether to inline imported modules into the importing file \[default\: true\]]:: :(true false)' \
'--sourcemap=[Generate source map. Accepted values are '\''linked'\'', '\''inline'\'', or '\''external'\'']:: :_default' \
...
'--check-js[Enable type-checking of JavaScript files (equivalent to \`compilerOptions.checkJs\: true\`)]' \
'*::file:_files' \
&& ret=0
;;
(clean)
...
'-q[Suppress diagnostic output]' \
'--quiet[Suppress diagnostic output]' \
'(--jsr)--npm[Treat unprefixed package names as npm packages]' \
'(--npm)--jsr[Treat unprefixed package names as JSR packages]' \
'-y[Bypass the prompt and run with full permissions]' \
'--yes[Bypass the prompt and run with full permissions]' \
'::package:_default' \
'*::package_args:_default' \
&& ret=0
;;
(completions)
_arguments "${_arguments_options[@]}" : \
...
'*--watch-exclude=[Exclude provided files/patterns from watch mode]::FILES:_files' \
'--use-tabs=[Use tabs instead of spaces for indentation \[default\: false\]]:: :(true false)' \
'--line-width=[Define maximum line width \[default\: 80\]]: :_default' \
'--indent-width=[Define indentation width \[default\: 2\]]: :_default' \
'--single-quote=[Use single quotes \[default\: false\]]:: :(true false)' \
'--prose-wrap=[Define how prose should be wrapped \[default\: always\]]: :(always never preserve)' \
'--no-semicolons=[Don'\''t use semicolons except where necessary \[default\: false\]]:: :(true false)' \
'--unstable[The \`--unstable\` flag has been deprecated. Use granular \`--unstable-*\` flags instead   To view the list of individual unstable feature flags, run this command again with --help=unstable]' \
'--unstable-bare-node-builtins[Enable unstable bare node builtins feature]' \
'--unstable-broadcast-channel[Enable unstable \`BroadcastChannel\` API]' \
'--unstable-bundle[Enable unstable bundle runtime API]' \
...
'-q[Suppress diagnostic output]' \
'--quiet[Suppress diagnostic output]' \
'(--lib --serve --empty --jsr)--npm[Generate a npm create-* project]' \
'(--lib --serve --empty --npm)--jsr[Generate a project from a JSR package]' \
'--lib[Generate an example library project]' \
'(--lib)--serve[Generate an example project for \`deno serve\`]' \
'(--lib --serve --npm --jsr)--empty[Generate a minimal project with just main.ts and deno.json]' \
'-y[Bypass the prompt and run with full permissions]' \
'--yes[Bypass the prompt and run with full permissions]' \
'*::args:_default' \
&& ret=0
...
'(-e --entrypoint -g --global)--save-exact[Save exact version without the caret (^)]' \
...
'--fail-fast=[Stop after N errors. Defaults to stopping after first failure]::N:_default' \
'--filter=[Run tests with this string or regexp pattern in the test name]: :_default' \
'--shuffle=[Shuffle the order in which the tests are run]::NUMBER:_default' \
'(--inspect --inspect-wait --inspect-brk)--coverage=[Collect coverage profile data into DIR. If DIR is not specified, it uses '\''coverage/'\''.   This option can also be set via the DENO_COVERAGE_DIR environment variable.]::DIR:_default' \
'(--no-run --coverage)*--watch=[Watch for file changes and restart process automatically.   Local files from entry point module graph are watched by default.   Additional paths might be watched by passing them as arguments to this flag.]::FILES:_files' \
'*--watch-exclude=[Exclude provided files/patterns from watch mode]::FILES:_files' \
...
'--checksum=[Verify the downloaded archive against the provided SHA256 checksum]: :_default' \
'--cert=[Load certificate authority from PEM encoded file]:FILE:_files' \
'--unsafely-ignore-certificate-errors=[DANGER\: Disables verification of TLS certificates]::HOSTNAMES:_default' \
'--unstable[The \`--unstable\` flag has been deprecated. Use granular \`--unstable-*\` flags instead   To view the list of individual unstable feature flags, run this command again with --help=unstable]' \
'--unstable-bare-node-builtins[Enable unstable bare node builtins feature]' \
...
'::version-or-hash-or-channel -- Version (v1.46.0), channel (alpha, beta, rc, canary) or commit hash (9bc2dd29ad6ba334fd57a20114e367d3c04763d4):_default' \
&& ret=0
;;
(vendor)
_arguments "${_arguments_options[@]}" : \
...
'(--no-check)--check=[Enable type-checking. This subcommand does not type-check by default   If the value of "all" is supplied, remote modules will be included.   Alternatively, the '\''deno check'\'' subcommand can be used]::CHECK_TYPE:_default' \
'*--env-file=[Load environment variables from local file   Only the first environment variable with a given key is used.   Existing process environment variables are not overwritten, so if variables with the same names already exist in the environment, their values will be preserved.   Where multiple declarations for the same environment variable exist in your .env file, the first one encountered is applied. This is determined by the order of the files you pass as arguments.]::FILE:_files' \
'()--install-alias=[Creates a dx alias so you can run dx <command> instead of deno x <command>]:: :_default' \
'--unstable[The \`--unstable\` flag has been deprecated. Use granular \`--unstable-*\` flags instead   To view the list of individual unstable feature flags, run this command again with --help=unstable]' \
'--unstable-bare-node-builtins[Enable unstable bare node builtins feature]' \
'--unstable-broadcast-channel[Enable unstable \`BroadcastChannel\` API]' \
'--unstable-bundle[Enable unstable bundle runtime API]' \
...
'create:scaffolds a project from a package' \
...
(( $+functions[_deno__create_commands] )) ||
_deno__create_commands() {
    local commands; commands=()
    _describe -t commands 'deno create commands' commands "$@"
}
...
(( $+functions[_deno__help__create_commands] )) ||
_deno__help__create_commands() {
    local commands; commands=()
    _describe -t commands 'deno help create commands' commands "$@"
}
